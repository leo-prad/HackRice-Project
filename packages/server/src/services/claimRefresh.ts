import { query } from "../db.js";
import { getPull, getPullReviews, parsePullUrl } from "./github.js";
import { completeClaim, markClaimClosed } from "./xp.js";

/**
 * Poll every submitted claim's PR. Merge or APPROVED review releases XP via
 * completeClaim; closed-without-merge transitions to 'closed' with no payout.
 */
export async function refreshSubmittedClaimsForUser(userId: number) {
  const tokenRow = await query<{ github_token: string }>("SELECT github_token FROM users WHERE id=$1", [userId]);
  const token = tokenRow.rows[0]?.github_token;
  if (!token) return { merged: 0, closed: 0 };
  const claims = await query<{ id: number; pr_url: string | null }>(
    "SELECT id, pr_url FROM claims WHERE user_id=$1 AND status='submitted' AND pr_url IS NOT NULL",
    [userId],
  );
  let merged = 0;
  let closed = 0;
  for (const row of claims.rows) {
    try {
      const parsed = parsePullUrl(row.pr_url!);
      try {
        const pull = await getPull(parsed.owner, parsed.repo, parsed.number, token);
        if (pull.merged) {
          await completeClaim(row.id, `PR merged: ${parsed.owner}/${parsed.repo}#${parsed.number}`);
          merged += 1;
        } else if (pull.state === "closed") {
          await markClaimClosed(row.id, userId);
          closed += 1;
        } else {
          const reviews = await getPullReviews(parsed.owner, parsed.repo, parsed.number, token).catch(() => []);
          if (reviews.some((review) => review.state === "APPROVED")) {
            await completeClaim(row.id, `PR approved: ${parsed.owner}/${parsed.repo}#${parsed.number}`);
            merged += 1;
          }
        }
      } catch (error) {
        if (error instanceof Error && /GitHub 404/.test(error.message)) {
          await markClaimClosed(row.id, userId);
          closed += 1;
        }
      }
    } catch { /* leave the claim for a later sweep */ }
  }
  return { merged, closed };
}
